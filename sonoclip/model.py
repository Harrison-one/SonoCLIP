...
def _make_identity_depthwise_weight(channels: int, kernel_size: int, dtype=None, device=None):
    weight = torch.zeros((channels, 1, kernel_size, kernel_size), dtype=dtype, device=device)
    weight[:, 0, kernel_size // 2, kernel_size // 2] = 1.0
    return weight
...
class VisionTransformer(nn.Module):
    def __init__(self, input_resolution: int, patch_size: int, width: int, layers: int, heads: int, output_dim: int, lora_adapt=False, rank=16):
        ...
        self.dwconv = nn.Conv2d(in_channels=width, out_channels=width, kernel_size=3, padding=1, groups=width, bias=False)
        with torch.no_grad():
            self.dwconv.weight.copy_(_make_identity_depthwise_weight(
                width,
                self.dwconv.kernel_size[0],
                dtype=self.dwconv.weight.dtype,
                device=self.dwconv.weight.device,
            ))
...
def build_model(state_dict: dict, lora_adapt=False, rank=16, alpha_in_channels: int = 1):
    ...
    if vit and 'visual.dwconv.weight' not in state_dict.keys():
        rgb_weight = state_dict['visual.conv1.weight']
        state_dict['visual.dwconv.weight'] = _make_identity_depthwise_weight(
            vision_width,
            3,
            dtype=rgb_weight.dtype,
            device=rgb_weight.device,
        )
