...
def load(name: str, alpha_vision_ckpt_pth="None", device: Union[str, torch.device] = "cuda" if torch.cuda.is_available() else "cpu", jit: bool = False, download_root: str = None, lora_adapt=False, rank=16, alpha_in_channels: int = 1):
    """Load a CLIP model
    ...
    if not jit:
        model = build_model(state_dict or model.state_dict(), lora_adapt=lora_adapt, rank=rank, alpha_in_channels=alpha_in_channels).to(device)
        if str(device) == "cpu":
            model.float()
        ...
        if alpha_vision_ckpt_pth != "None":
            ...
            model.visual.load_state_dict(torch.load(alpha_vision_ckpt_pth), strict=False)
